...
from rpgtools import db, bcrypt
from rpgtools.models import User, Post
from rpgtools.users.forms import (RegistrationForm, LoginForm, UpdateAccountForm,
                                   RequestResetForm, GrantResetForm)
...
@users.route('/usr/<string:username>')
...
def grant_reset(token):
    ...
    if user is None:
        ...
        return redirect(url_for('uses.request_reset'))
    form = GrantResetForm()
    if form.validate_on_submit():
        hashed_pw = bcrypt.generate_password_hash(form.password.data).decode('utf-8)')
        ...
        return redirect(url_for('uses.login'))
